...
module.exports = async (ctx) => {
    ...
    const emoji_api = `https://mashiro.top/bilibili_emojis/index.json`;
    const emoji_list = await ctx.cache.tryGet(emoji_api, async () => {
        const result = await got.get(emoji_api);
        return result.data;
    });
        ...
        item: data.map((item) => {
            ...
            if (item.display.emoji_info) {
                ...
                emoji.forEach(function (item) {
                    ...
                    if (short_code in emoji_list) {
                        data_content = data_content.replace(new RegExp(`\\${item.text}`, 'g'), ` :${emoji_list[short_code]}: `);
                    }
                });
            }
            const getUrl = (data) => {
